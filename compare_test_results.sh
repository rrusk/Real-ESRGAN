...
# Description: Generic 4-way comparison for test clips.
# ==============================================================================
set -euo pipefail
...
# 4. Grid Config (4x Native Resolution)
WIDTH=1408
HEIGHT=960
...
-i "$INPUT_X2" \
-filter_complex " \
    color=s=${WIDTH}x${HEIGHT}:c=black[blank]; \
    [0:v]scale=${WIDTH}:${HEIGHT}:flags=neighbor,drawtext=text='Original VHS':x=20:y=20:fontsize=60:fontcolor=white:box=1:boxcolor=black@0.5[tl]; \
    [1:v]drawtext=text='4x (Filtered, No Face)':x=20:y=20:fontsize=60:fontcolor=white:box=1:boxcolor=black@0.5[tr]; \
    [2:v]scale=${WIDTH}:${HEIGHT}:flags=lanczos,drawtext=text='2x (Filtered, No Face)':x=20:y=20:fontsize=60:fontcolor=white:box=1:boxcolor=black@0.5[br]; \
    [blank]drawtext=text='Empty Panel':x=20:y=20:fontsize=60:fontcolor=white[bl]; \
    [tl][tr]hstack[top]; \
    [bl][br]hstack[bottom]; \
    [top][bottom]vstack[v] \
" \
