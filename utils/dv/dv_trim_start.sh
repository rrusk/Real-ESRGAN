#!/bin/bash
# ==============================================================================
# Script Name: dv_trim_start.sh
# Description: Trims the start of a DV1 file at a specified time using stream
...
#
# Usage:
#   ./dv_trim_start.sh <start_time> <input.dv>
#
# Arguments:
#   start_time  Where to start, discarding everything before this point.
#               Accepts HH:MM:SS, MM:SS, or raw seconds (decimals ok).
#   input.dv    Source DV1 file.
#
# Examples:
#   ./dv_trim_start.sh 0:08 hi8_20011102.dv
#   ./dv_trim_start.sh 8.0  hi8_20011102.dv
# ==============================================================================
set -euo pipefail
...
if [[ $# -ne 2 || "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
    echo "Usage: $0 <start_time> <input.dv>"
    echo "  start_time: HH:MM:SS, MM:SS, or seconds"
...
# -ss after -i (output seek): ffmpeg decodes and discards frames up to
# START_TIME rather than seeking in the container.  For AVI/DV1 this is
# necessary -- an input seek causes ffmpeg to rewrite the AVI container
# index from scratch, inflating the output beyond the original file size.
# Decoding a few seconds at the start is negligible.
# -c copy: stream copy after the seek point; preserves DV quality exactly.
