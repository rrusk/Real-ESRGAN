#!/bin/bash
# dvd_enhance.sh — Enhance and encode a DVD-extracted MPG to distribution-ready
# MP4 and MKV files (H.264 + AAC) with deinterlacing, noise reduction,
# Lanczos upscaling, and embedded chapter markers.
#
# This script is the final step in the DVD preservation pipeline:
#
#   dvd_extract.sh  ->  dvd_enhance.sh
#
...
#
# Usage:
#   ./dvd_enhance.sh [OPTIONS] <input_video> <chapter_file>
#
# Arguments:
...
#   --crf N          H.264 CRF quality (default: 16 — near-transparent for SD).
#                    23 is ffmpeg default.  Lower = larger file.
#   --preset NAME    H.264 encoding preset (default: slow).
#                    Choices: ultrafast fast medium slow slower veryslow
#   -o, --output FILE  Output MP4 filename.
#                    Default: <input_stem>.mp4 and <input_stem>.mkv
...
if [[ -z "$CHAPTER_FILE" && -n "$NAMES_FILE" ]]; then
    ...
    # Also search without any suffix the user may have added.
    if [[ ${#CANDIDATE_OGMS[@]} -eq 0 ]]; then
        BARE_STEM="${INPUT_BASE_STEM}"
        mapfile -t CANDIDATE_OGMS < <(
...
if ! [[ "$CRF" =~ ^[0-9]+$ ]] || (( CRF > 51 )); then
    echo "[ERROR] CRF must be an integer 0-51 (recommended: 14-18 for archival SD)."
    exit 1
fi
...
echo ""
echo "========================================="
echo "  dvd_enhance.sh -- dependency check"
echo "========================================="
echo ""
...
#
# Tier priority (matches enhance_video.sh):
...
#
# Detection priority (matches enhance_video.sh and prepare_video.sh):
...
#
# Priority (matches enhance_video.sh):
#   1. --dar override
...
if [[ -n "$NAMES_FILE" ]]; then
    ...
    OGM_NAMED=$(mktemp /tmp/dvd_enhance_ogm_XXXXXX.txt)
...
CHAP_XML=$(mktemp /tmp/dvd_enhance_chap_XXXXXX.xml)
...
# x264 at CRF 16 typically produces 50-70% of source DVD bitrate.
...
if [[ "$OUTPUT_MODE" != "mkvonly" ]]; then
    ...
    if [[ $HAVE_MP4BOX -eq 0 ]]; then
        ...
        MP4_CHAP_TMP=$(mktemp /tmp/dvd_enhance_mp4chap_XXXXXX.txt)
        awk '
            /^CHAPTER[0-9]+=/ && !/NAME/ {
        ...
        CHAPTERED_TMP=$(mktemp /tmp/dvd_enhance_chaptered_XXXXXX.mp4)
